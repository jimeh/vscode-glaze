...
export function getTintConfig(): TintConfig {
  ...
  const allowedBaseHues = _validateAllowedBaseHues(
    config.get<unknown>('tint.allowedBaseHues', [])
  );
  const customBaseColors = _validateCustomBaseColors(
    config.get<unknown>('tint.customBaseColors', [])
  );
